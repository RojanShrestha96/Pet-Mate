...
export function HomePage() {
  ...
  const featuredPets = mockPets
    .filter((p) => p.adoptionStatus === "available")
    .slice(0, 6);
  ...
  useEffect(() => {
    const fetchShelters = async () => {
      try {
        ...
        // We need a route for this. Assuming /api/shelter returns a list.
        const res = await axios.get("http://localhost:5000/api/shelter");
