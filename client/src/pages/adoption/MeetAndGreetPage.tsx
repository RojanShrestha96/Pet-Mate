...
import {
  MeetAndGreetModal,
  MeetAndGreetData,
} from "../../components/adoption/MeetAndGreetModal";
...
export function MeetAndGreetPage() {
  ...
  const [selectedMeeting, setSelectedMeeting] = useState<MeetAndGreet | null>(
    null
  );
  ...
  const [meetAndGreets, setMeetAndGreets] = useState<MeetAndGreet[]>([
    ...
    },
    {
      ...
      petName: "Max",
      petImage:
        "https://images.unsplash.com/photo-1587300003388-59208cc962cb?w=100",
      date: "2024-12-15",
      time: "2:30 PM",
      location: "Loading...",
      status: "scheduled",
  ...
  const handleCompleteConfirm = () => {
    if (selectedMeeting) {
      ...
      showToast(
        "Meeting confirmed! Moving application to Adoption Finalization.",
        "success"
      );
  ...
  const activeMeetings = meetAndGreets.filter(
          ...
          <div className="flex-shrink-0 grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card className="p-3 flex items-center justify-between shadow-sm">
              <div>
                ...
                <p className="text-xl font-bold text-gray-900">
                  {
                    activeMeetings.filter((m) => isSameDay(new Date(), m.date))
                      .length
                  }
                </p>
              </div>
            ...
            <Card className="p-3 flex items-center justify-between shadow-sm">
              <div>
                ...
                <p className="text-xl font-bold text-gray-900">
                  {
                    activeMeetings.filter((m) => m.status === "scheduled")
                      .length
                  }
                </p>
              </div>
              ...
              <Card className="flex-1 flex flex-col overflow-hidden shadow-sm border border-gray-200">
                ...
                <div className="flex-1 grid grid-cols-7 grid-rows-6 divide-x divide-gray-100 divide-y bg-white min-h-0">
                  {getDaysInMonth(currentDate).map((date, idx) => {
                    if (!date)
                      return (
                        <div key={`empty-${idx}`} className="bg-gray-50/30" />
                      );
                    ...
                    return (
                      <div
                        ...
                        className={`p-1 relative group hover:bg-gray-50 transition-colors flex flex-col ${
