...
export function AddPetPage() {
  ...
  const STORAGE_KEY = 'addPetFormData';

  const [currentStep, setCurrentStep] = useState(1);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [showConfirm, setShowConfirm] = useState(false);
  
  // Initialize form data from localStorage if available (survives page refresh)
  const [formData, setFormData] = useState<FormData>(() => {
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      if (saved) {
        const parsed = JSON.parse(saved);
        // File objects cannot be stored in localStorage, so images/documents must be re-uploaded
        return {
          ...initialFormData,
          ...parsed,
          images: [],
  ...
  useEffect(() => {
    try {
      const dataToSave = {
        ...formData,
        images: [], // Files can't be serialized to localStorage
        medicalDocuments: []
      };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      clearSavedFormData(); // Clear saved form data on success
