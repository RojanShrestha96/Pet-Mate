...
export function ShelterProfilePage() {
  ...
  // Fallback to mock pets for now (replace with real fetch later if available)
  // Logic: Show random subset or all since we don't have real pets tailored to this ID in mockData usually
  const shelterPets = mockPets; // .filter(pet => pet.shelterId === id);
                  ...
                  <Card className="p-6 md:p-8 border border-[var(--color-border)]">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                      ...
                      <div className="flex gap-2 overflow-x-auto pb-2 sm:pb-0">
                        {(["all", "dogs", "cats", "other"] as const).map(
                          (tab) => (
                            <button
                              key={tab}
